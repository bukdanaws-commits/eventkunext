...
export default function AdminEmailTrackingDetail() {
  ...
  // Get unique URLs with click counts
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className={`p-4 rounded-lg border ${broadcast.winner_variant === 'A' ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20' : 'bg-blue-50 dark:bg-blue-900/20'}`}>
                  ...
                  <div className="mt-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      ({((trackingData.filter(t => t.variant === 'A' && t.opened_at).length / (trackingData.filter(t => t.variant === 'A').length || 1)) * 100).toFixed(1)}%)
                    </div>
                  </div>
                </div>
                <div className={`p-4 rounded-lg border ${broadcast.winner_variant === 'B' ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20' : 'bg-purple-50 dark:bg-purple-900/20'}`}>
                  ...
                  <div className="mt-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      ({((trackingData.filter(t => t.variant === 'B' && t.opened_at).length / (trackingData.filter(t => t.variant === 'B').length || 1)) * 100).toFixed(1)}%)
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Location Stats & Click Analytics */}
        <div className="grid md:grid-cols-2 gap-4">
          {/* Location Stats */}
          <Card>
            <CardHeader>
                                      ...
                                      <div className="p-3 rounded-lg bg-muted">
                                        ...
                                        {tracking.latitude && tracking.longitude && (
                                          <div className="text-xs text-muted-foreground mt-1">
                                            Coordinates: {tracking.latitude.toFixed(4)}, {tracking.longitude.toFixed(4)}
                                          </div>
                                        )}
                                      </div>
                                    )}
