...
export default function EventDrawDisplay() {
  ...
  const handleStateChange = useCallback((state: DrawState) => {
    ...
    // If no new state within 30 seconds (3s countdown + 27s buffer for DB query + network), return to idle
  ...
  useEffect(() => {
    if (connectionStatus === 'connected') {
      ...
      // Small delay to ensure operator channel is ready
  ...
  const renderAnimation = () => {
    // Don't block animation if we have a winner - use fallback participants if needed
    const participantsToUse = drawState.participants.length > 0 
      ? drawState.participants 
      : drawState.winner 
        ? [drawState.winner] // Use winner as minimum participant for animation
        : [];
