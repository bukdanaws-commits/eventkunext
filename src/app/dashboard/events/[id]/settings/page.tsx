...
function EventSettingsContent({ event, refetchEvent }: EventContextValue) {
  ...
  const [slugInput, setSlugInput] = useState(event.public_viewer_slug || '');
  ...
  useEffect(() => {
    if (window.location.hash === '#form-builder' && formBuilderRef.current) {
      setTimeout(() => {
  ...
  const handleSaveSlug = async () => {
    ...
    if (error) {
      ...
      setLocalEvent(prev => ({ ...prev, public_viewer_slug: slugInput }));
      toast({
        title: 'Berhasil',
  ...
  const handleToggleQR = async (checked: boolean) => {
    ...
    if (error) {
      ...
      setLocalEvent(prev => ({ ...prev, qr_checkin_enabled: checked }));
      toast({
  ...
  const handleToggleCheckinRequired = async (checked: boolean) => {
    ...
    if (error) {
      ...
      setLocalEvent(prev => ({ ...prev, checkin_required_for_draw: checked }));
      toast({
  ...
  const handleToggleEmailNotification = async (checked: boolean) => {
    ...
    if (error) {
      ...
      setLocalEvent(prev => ({ ...prev, email_notification_enabled: checked }));
      toast({
  ...
  const handleTogglePaidEvent = async (checked: boolean) => {
    ...
    if (error) {
      ...
      setLocalEvent(prev => ({ ...prev, is_paid_event: checked }));
      toast({
  ...
  const handleToggleRegistration = async (checked: boolean) => {
    ...
    if (error) {
      ...
      setLocalEvent(prev => ({ ...prev, registration_enabled: checked }));
      toast({
  ...
  const copyLink = () => {
    navigator.clipboard.writeText(`${window.location.origin}/live/${localEvent.public_viewer_slug}`);
    setCopied(true);
    toast({
      title: 'Link disalin',
  ...
  const registrationLink = `${window.location.origin}/register/${localEvent.public_viewer_slug}`;
  ...
  const previewRegistration = () => {
    window.open(`/register/${localEvent.public_viewer_slug}`, '_blank');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-4">
            ...
            {localEvent.public_viewer_slug && (
              <div className="flex items-center gap-2 p-3 bg-muted rounded-lg">
                <code className="flex-1 text-sm truncate">
                  {window.location.origin}/live/{localEvent.public_viewer_slug}
                </code>
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => window.open(`/live/${localEvent.public_viewer_slug}`, '_blank')}
                >
        ...
        <Card className="md:col-span-2">
              ...
              {localEvent.public_viewer_slug && (
                <div className="flex items-center gap-2">
                  <Label htmlFor="registration-toggle" className="text-sm">
                    {localEvent.registration_enabled ? 'Buka' : 'Tutup'}
                  </Label>
                  <Switch
                    id="registration-toggle"
                    checked={localEvent.registration_enabled ?? true}
          ...
          <CardContent className="space-y-4">
            {localEvent.public_viewer_slug ? (
              <>
                {!localEvent.registration_enabled && (
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="qr-toggle" className="flex flex-col gap-1">
                ...
                <span className="text-sm font-normal text-muted-foreground">
                  {localEvent.qr_checkin_enabled ? 'Peserta dapat check-in dengan QR' : 'QR check-in dinonaktifkan'}
                </span>
              </Label>
              <Switch
                id="qr-toggle"
                checked={localEvent.qr_checkin_enabled}
                onCheckedChange={handleToggleQR}
              />
            </div>
            {localEvent.qr_checkin_enabled && (
              <Button 
                variant="outline" 
                className="w-full"
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <Label htmlFor="checkin-required-toggle" className="flex flex-col gap-1">
                ...
                <span className="text-sm font-normal text-muted-foreground">
                  {localEvent.checkin_required_for_draw 
              ...
              <Switch
                ...
                checked={localEvent.checkin_required_for_draw}
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <Label htmlFor="email-toggle" className="flex flex-col gap-1">
                ...
                <span className="text-sm font-normal text-muted-foreground">
                  {localEvent.email_notification_enabled 
              ...
              <Switch
                id="email-toggle"
                checked={localEvent.email_notification_enabled}
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              ...
              <Switch
                id="paid-toggle"
                checked={localEvent.is_paid_event || false}
      ...
      {localEvent.public_viewer_slug && (
        <FormThemeSelector 
          eventId={event.id} 
          currentTheme={localEvent.form_theme as FormThemeId}
          onThemeChange={(theme) => setLocalEvent(prev => ({ ...prev, form_theme: theme }))}
        />
      )}
      ...
      {localEvent.is_paid_event && (
        <TicketTierManagement eventId={event.id} isPaidEvent={localEvent.is_paid_event || false} />
      )}
...
export default function EventSettings() {
  return (
    <EventLayout>
      {(context) => <EventSettingsContent {...context} />}
    </EventLayout>
  );
}
