...
function EventDashboardContent({ event, tierInfo, participantsCount, prizesCount, winnersCount, prizeStats, winnerStats, recentWinners, refetchEvent }: EventContextValue) {
  ...
  useCheckinNotifications({ eventId: event.id, enabled: event.status === 'active' });
  ...
  const categoryLabels: Record<Database['public']['Enums']['prize_category'], string> = {
    hiburan: 'Hiburan',
    utama: 'Utama',
                ...
                <div key={winner.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{winner.prize?.name || 'Unknown Prize'}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <Badge variant="outline" className="text-xs">
                      {winner.prize?.category ? categoryLabels[winner.prize.category] : '-'}
                    </Badge>
...
export default function EventDashboard() {
  return (
    <EventLayout>
      {(context) => <EventDashboardContent {...context} />}
    </EventLayout>
  );
}
